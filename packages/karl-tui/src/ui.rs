...
fn draw_title_bar(f: &mut Frame, _app: &App, area: Rect) {
    let title = Paragraph::new(Line::from(vec![
        Span::styled(" karl ", theme::title_style()),
...
fn draw_wizard_welcome(f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        Line::from(""),
        Line::from(Span::styled("Welcome to Karl!", Style::default().fg(theme::PRIMARY).add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from("This wizard will help you set up your first provider and model."),
        Line::from(""),
        Line::from("Karl supports multiple AI providers:"),
        Line::from(vec![
            Span::raw("  • "),
...
fn draw_wizard_oauth(f: &mut Frame, wizard: &crate::app::InitWizard, area: Rect) {
    ...
    match &wizard.oauth_status {
        OAuthStatus::NotStarted => {
            ...
            lines.push(Line::from(""));
            lines.push(Line::from("You'll be asked to authorize Karl to use your account."));
            lines.push(Line::from(""));
...
fn draw_wizard_confirm(f: &mut Frame, wizard: &crate::app::InitWizard, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("Config will be saved to ~/.config/karl/karl.json", theme::dim_style())),
        Line::from(""),
