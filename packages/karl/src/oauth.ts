...
export function loadOAuthCredentials(provider: string): OAuthCredentials | null {
  // First check Karl's own storage
...
async function ensureClaudeProMaxProvider(): Promise<void> {
  const configPath = resolveHomePath('~/.config/karl/karl.json');
...
export async function runLoginFlow(): Promise<void> {
  ...
  console.log('\n🎾 Karl OAuth Login\n');
  console.log('This will open your browser to authorize Karl with your Claude account.');
