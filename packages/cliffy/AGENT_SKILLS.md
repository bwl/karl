# Agent Skills Implementation in Cliffy

Cliffy now supports the [Agent Skills](https://agentskills.io) open standard for extending AI agent capabilities with specialized knowledge and workflows.
...
cliffy --skill code-review "review the auth.go file"
cliffy --skill documentation "document the API endpoints in api.go"
```
...
cliffy skills create my-skill

# Create with custom description
cliffy skills create data-analysis --description "Analyze datasets and generate insights"

# Create at specific path
cliffy skills create custom-workflow --path ./skills/custom-workflow
```
...
Cliffy looks for skills in these directories (in order):

1. `~/.config/cliffy/skills/` - Global skills
2. `./.cliffy/skills/` - Project-specific skills
...
Cliffy includes several example skills:
...
```bash
cliffy --skill security-review "analyze the authentication system"
```
...
```bash
cliffy --skill code-review "review the changes in the last commit"
```
...
```bash
cliffy skills create my-custom-skill --description "My specialized workflow"
```
...
```bash
# Validate the skill
cliffy skills validate ./my-custom-skill

# Test the skill
cliffy --skill my-custom-skill "test task for my skill"
```
...
Place skills in `./.cliffy/skills/` for project-specific workflows:
...
## Integration with Cliffy

Skills integrate seamlessly with Cliffy's existing features:

- **Volley Mode**: Use skills with parallel task execution
- **Context Loading**: Skills work with project context files
- **Tool System**: Skills can leverage all of Cliffy's tools
- **Hooks**: Skills can trigger custom hooks for automation

## Contributing Skills

To contribute skills to the Cliffy ecosystem:
...
1. Create a new skill with `cliffy skills create`
