/**
 * Config Stack management for Cliffy
 *
...
import type { StackConfig, CliffyConfig, CliOptions } from './types';
...
export class StackManager {
  ...
  constructor(config: CliffyConfig, customPaths: string[] = []) {
    this.inlineStacks = config.stacks || {};
    this.stacksPaths = [
      // Standard paths
      join(homedir(), '.config', 'cliffy', 'stacks'),
      join(process.cwd(), '.cliffy', 'stacks'),
      // Custom paths
      ...customPaths
    ];
  }
...
export function getStackManager(config: CliffyConfig): StackManager {
  if (!defaultManager) {
