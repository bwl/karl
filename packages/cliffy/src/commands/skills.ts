...
export async function listSkills(options: SkillsListOptions = {}) {
  ...
  if (skills.length === 0) {
    console.log('No skills found. Create skills in ~/.config/cliffy/skills/ or ./.cliffy/skills/');
    return;
  }
...
export async function createSkill(options: SkillsCreateOptions) {
  ...
  try {
    ...
    console.log(`4. Test your skill with: cliffy skills validate ${skillPath}`);
    console.log(`5. Use your skill with: cliffy --skill ${options.name} "your task"`);
...
\`\`\`bash
cliffy --skill ${name} "your task description"
\`\`\`
...
export async function handleSkillsCommand(args: string[]) {
  ...
  switch (command) {
    ...
    case 'info':
      if (rest.length === 0) {
        console.error('Usage: cliffy skills show <skill-name>');
        process.exit(1);
      }
    ...
    case 'new':
      if (rest.length === 0) {
        console.error('Usage: cliffy skills create <skill-name> [--description "..."] [--path /path/to/skill]');
        process.exit(1);
      }
    ...
    case 'check':
      if (rest.length === 0) {
        console.error('Usage: cliffy skills validate <skill-path>');
        process.exit(1);
      }
