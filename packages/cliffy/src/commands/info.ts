 ...
 * Used by cliffy-tui to display status information
 */
...
export async function getInfo(cwd: string): Promise<InfoOutput> {
  ...
  const projectPath = join(cwd, '.cliffy.json');
  ...
  let hookCount = 0;
  const hookPaths = [
    resolveHomePath('~/.config/cliffy/hooks'),
    join(cwd, '.cliffy', 'hooks')
  ];
...
export async function handleInfoCommand(args: string[]): Promise<void> {
  ...
  if (isJson) {
    ...
    console.log(`Cliffy v${info.version}\n`);
