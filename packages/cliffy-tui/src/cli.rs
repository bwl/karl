...
//!
//! Calls the cliffy CLI to get system info, auth status, etc.
...
/// Info output from `cliffy info --json`
...
/// Check if cliffy CLI is available
...
pub fn get_cli_info() -> Result<CliInfo> {
    let output = Command::new("cliffy")
        .args(["info", "--json"])
        .output()
        .context("Failed to run cliffy info")?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        anyhow::bail!("cliffy info failed: {}", stderr);
    }

    let info: CliInfo = serde_json::from_slice(&output.stdout)
        .context("Failed to parse cliffy info output")?;
...
pub fn run_interactive_command(args: &[&str]) -> Result<bool> {
    let status = Command::new("cliffy")
        .args(args)
        .stdin(std::process::Stdio::inherit())
        .stdout(std::process::Stdio::inherit())
        .stderr(std::process::Stdio::inherit())
        .status()
        .context("Failed to run cliffy command")?;
...
mod tests {
    ...
    fn test_check_cli_available() {
        // This test will pass if cliffy is in PATH
    ...
    fn test_parse_cli_info() {
        let json = r#"{
            "version": "0.1.0",
            "config": {
                "global_path": "/home/user/.config/cliffy/cliffy.json",
                "project_path": ".cliffy.json",
                "global_exists": true,
